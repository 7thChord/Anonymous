...
class LatteT2V(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        temp_pos_embed = get_1d_sincos_pos_embed(inner_dim, self.video_length, interpolation_scale=interpolation_scale_1d)  # 1152 hidden size
        ...
        # if num CA lora layers is less than 28 than we only apply LORA to the first half or last half of the CA blocks
        if num_CA_lora_layers < 28:
            print("[Partial CA Applied]")
            self.transformer_blocks = nn.ModuleList(
                [
                    BasicTransformerBlock(
                        ...
                        use_spatial_lora = use_spatial_lora,
                        use_CA_lora = use_CA_lora,
                        use_multi_ca = use_multi_ca,
                        lora_rank = lora_rank,
                    ...
                    for d in range(num_CA_lora_layers)
                ]
            ...
            self.transformer_blocks.extend(       
                [    
                    ...
                    for d in range(num_layers - num_CA_lora_layers)
                ]
            )
        ...
        self.temporal_transformer_blocks = nn.ModuleList(
            [
                BasicTransformerBlock_(  # one attention
                    ...
                    use_temporal_lora = use_temporal_lora,
                    lora_rank = lora_rank,
                    num_adapters = num_adapters,
                    adapter_weights = adapter_weights
                )
    ...
    def vae_to_diff_mask(self, attention_mask, use_image_num):
        ...
        attention_mask = F.max_pool2d(attention_mask.float(), kernel_size=(self.patch_size, self.patch_size), stride=(self.patch_size, self.patch_size))
    ...
    def forward(
        ...
        #all of this for getting the 'attention_mask'
        ...
        # all of this for getting the 'encoder_attention_mask'
        ...
        if self.caption_projection is not None:
            batch_size = hidden_states.shape[0]
            # Shape of encoder hidden state [[1, 1, 300, 4096]]
            encoder_hidden_states = self.caption_projection(encoder_hidden_states.to(self.dtype))  # [1 1 300 1152]
            ...
            if use_image_num != 0 and self.training:
                ...
                if not self.q_downsample:
                    encoder_hidden_states_spatial = repeat(encoder_hidden_states, 'b 1 t d -> (b f) t d', f=frame).contiguous() # [5, 300, 1152]
                else:
                   encoder_hidden_states_spatial = rearrange(encoder_hidden_states, 'b f t d -> (b f) t d').contiguous() # [1, 300, 1152]
    ...
    @classmethod
    def from_pretrained_multiadapter(cls,pretrained_model_path,adapter_ckpt_paths,adapter_weights, subfolder=None,args=None,**kwargs):
        assert type(adapter_ckpt_paths) == list
        if subfolder is not None:
            pretrained_model_path = os.path.join(pretrained_model_path, subfolder)
        ...
        with open(config_file, "r") as f:
            config = json.load(f)
        config['num_adapters'] = len(adapter_ckpt_paths)
        config['adapter_weights'] = adapter_weights
        config['use_multi_ca'] = args.use_multi_ca
        assert len(config['adapter_weights']) == len(adapter_ckpt_paths)
        num_adapters = len(adapter_ckpt_paths)
        model = cls.from_config(config, **kwargs)
        from diffusers.utils import WEIGHTS_NAME
        model_file = os.path.join(pretrained_model_path,WEIGHTS_NAME).replace("bin","safetensors") #weights name

        state_dict = {}
        with safe_open(model_file, framework="pt", device="cpu") as f:
            for key in f.keys():
                if 'lora' not in key and 'i2v_adapter' not in key:
                    state_dict[key] = f.get_tensor(key)

        adapter_weight_names = ['processor.q_lora','processor.k_lora','processor.v_lora','processor.out_lora']
        for i,ckpt_path in enumerate(adapter_ckpt_paths):
            adapter_state_dict ={}
            with safe_open(os.path.join(ckpt_path,WEIGHTS_NAME).replace("bin","safetensors"), framework="pt", device="cpu") as f:
                for key in f.keys():
                    if num_adapters > 1:
                        if 'lora' in key:
                            new_key = key.replace("lora",f"loras.{i}")
                            adapter_state_dict[new_key] = f.get_tensor(key)
                        if 'i2v_adapter' in key:
                            new_key = key.replace("i2v_adapter",f"i2v_adapters.{i}")
                            adapter_state_dict[new_key] = f.get_tensor(key)
                    else:
                        adapter_state_dict[key] = f.get_tensor(key)
            state_dict.update({name: param for name,param in adapter_state_dict.items()})
