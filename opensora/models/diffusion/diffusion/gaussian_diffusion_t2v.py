...
class GaussianDiffusion_T:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, use_first_frame_cond = False):
        """
        ...
        if noise is None:
            noise = th.randn_like(x_start)
        # Inject noise / Forward diffusion in latent domain
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            # import ipdb;ipdb.set_trace()
            if use_first_frame_cond: 
                terms["mse"] = mean_flat(((target[:,:,1:,:,:] - model_output[:,:,1:,:,:]) ** 2) * mask[:,:,1:,:,:])
            else:
